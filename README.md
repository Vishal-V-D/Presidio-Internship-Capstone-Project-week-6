...
```mermaid
graph LR
    FE[Frontend Client (React/Vite)] -->|REST & WebSocket| SUB[Submission Service :5000]
    FE -->|REST| UCON[User-Contest Service :4000]
    SUB -->|Problem metadata| UCON
    SUB -->|POST /api/ai/feedback| RAG[RAG Pipeline :8000]
    UCON -->|TypeORM| MYSQL[(MySQL - Users/Contests/Problems)]
    SUB -->|TypeORM| MYSQL_SUB[(MySQL - Submissions)]
